...
if ($mode == "add") {
    ...
    // $shipmentdate = explode("-", $_POST['shipmentdate']);

    // echo '<pre>',print_r($shipmentdate),'</pre'; exit();
    // $day = $shipmentdate[0];
    // $month = $shipmentdate[1];
    // $year = $shipmentdate[2];
    
    // $newshipmentdate = date('Y-m-d', strtotime($year.'-'. $month.'-'. $day));
    // echo $newshipmentdate; exit();
    ...
    $sqlH = "INSERT INTO tbordheader(ordid,supplierid,userid,statusid) 
                    VALUES('$ordid','$supplierid','$userid','$statusid')";
    // echo $sqlH; exit();
