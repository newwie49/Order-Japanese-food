<?php
@session_start();
$_SESSION['mode'] = "edit";
include '../db/linkdb.php';

$unitid = $unitname = $statusid = '';

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    
    // Use prepared statements to avoid SQL injection
    $stmt = $conn->prepare("SELECT un.unitid, un.unitname, un.statusid, st.statusname
                             FROM tbunit un
                             JOIN tbstatus st ON un.statusid = st.statusid
                             WHERE un.unitid = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $rs = $result->fetch_assoc();
        $unitid = $rs['unitid'];
        $unitname = $rs['unitname'];
        $statusid = $rs['statusid'];
    } else {
        // Handle the case where no unit is found
        echo "No unit found with the given ID.";
        exit();
    }
    $stmt->close();
    $conn->close();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Unit</title>
    <?php include_once('../center/linkcss.php');?>
</head>
<body>
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <?php include_once('../center/menu.php');?>
            <h1 class="text-center">แก้ไขข้อมูล</h1>
            <form action="saveunit.php" method="post">
                <div class="row">
                    <div class="col-4 offset-4">
                        <div class="input-group mt-3">
                            <span class="input-group-text" id="basic-addon1">รหัสหน่วยวัด</span>
                            <input type="text" class="form-control" placeholder="" name="unitid" id="unitid" value="<?php echo htmlspecialchars($unitid); ?>" readonly>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-4 offset-4">
                        <div class="input-group mt-3">
                            <span class="input-group-text" id="basic-addon1">ชื่อหน่วยวัด</span>
                            <input type="text" class="form-control" placeholder="" name="unitname" id="unitname" value="<?php echo htmlspecialchars($unitname); ?>">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-4 offset-4">
                        <div class="input-group mt-3">
                            <span class="input-group-text" id="basic-addon1">สถานะ</span>
                            <select name="statusid" id="statusid" class="form-control" required></select>
                        </div>
                    </div>
                </div>
                <div class="row mt-3">
                    <div class="col-4 offset-4">
                        <input type="submit" value="บันทึกข้อมูล" class="btn btn-primary">
                        <a href="manageunit.php" class="btn btn-secondary">ย้อนกลับ</a>             
                    </div>
                </div>    
            </form>
        </div>
    </div>
</div>
<?php include_once('../center/linkjs.php');?>
<script>
    $(document).ready(function() {
        $.ajax({
            url: 'getstatus.php',
            method: 'post',
            data: '',
            success: function(result) {
                $("#statusid").html(result);
                $("#statusid").val("<?php echo htmlspecialchars($statusid); ?>"); // Set the selected value
            }
        });
    });
</script>
</body>
</html>
